...
static esp_err_t sps30_write_cmd_with_args(uint16_t cmd, const uint8_t *args_words_crc, size_t args_len)
{
    // Build: [cmd_msb cmd_lsb] + args
    uint8_t buf[2 + 16] = {0};
    if (args_len > 16) return ESP_ERR_INVALID_SIZE;
    ...
    return i2c_master_write_to_device(I2C_PORT, SPS30_I2C_ADDR, buf, 2 + args_len, pdMS_TO_TICKS(200));
}
...
static esp_err_t sps30_read_words_with_crc(uint16_t cmd, uint8_t *out, size_t out_len)
{
    // Send cmd (2 bytes), then read out_len bytes
    uint8_t c[2] = {(uint8_t)(cmd >> 8), (uint8_t)(cmd & 0xFF)};

    esp_err_t err = i2c_master_write_to_device(I2C_PORT, SPS30_I2C_ADDR, c, 2, pdMS_TO_TICKS(200));
    if (err != ESP_OK) return err;

    // SPS30 needs small processing time for some commands; safe tiny delay
    vTaskDelay(pdMS_TO_TICKS(5));

    err = i2c_master_read_from_device(I2C_PORT, SPS30_I2C_ADDR, out, out_len, pdMS_TO_TICKS(300));
    return err;
}
...
static esp_err_t sps30_start_measurement(void)
{
    ...
    esp_err_t err = sps30_write_cmd_with_args(SPS30_CMD_START_MEASUREMENT, word, sizeof(word));
...
static esp_err_t sps30_read_data_ready(bool *ready)
{
    ...
    uint8_t rx[3] = {0};
    esp_err_t err = sps30_read_words_with_crc(SPS30_CMD_READ_DATA_READY, rx, sizeof(rx));
    if (err != ESP_OK) return err;

    if (sensirion_crc8(rx, 2) != rx[2]) return ESP_ERR_INVALID_CRC;
...
static esp_err_t sps30_read_measurement(float *vals_10)
{
    ...
    uint8_t rx[60] = {0};
    esp_err_t err = sps30_read_words_with_crc(SPS30_CMD_READ_MEASUREMENT, rx, sizeof(rx));
    if (err != ESP_OK) return err;
    ...
    for (int i = 0; i < 60; i += 3) {
        if (sensirion_crc8(&rx[i], 2) != rx[i + 2]) return ESP_ERR_INVALID_CRC;
        payload[p++] = rx[i];
        payload[p++] = rx[i + 1];
    }
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "I2C init OK (SDA=%d, SCL=%d, freq=%d Hz)", I2C_SDA_GPIO, I2C_SCL_GPIO, I2C_FREQ_HZ);
